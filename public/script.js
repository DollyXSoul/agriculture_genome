function showTab(id, btn) {
  document
    ...
    .forEach((n) => n.classList.remove("active")); // [MDN: classList]
  document
    .querySelectorAll(".tab-button")
    .forEach((n) => n.classList.remove("active")); // [MDN: classList]
  const pane = document.getElementById(id); // [MDN: getElementById]
  if (pane) pane.classList.add("active"); // [MDN: classList]
  if (btn) btn.classList.add("active"); // [MDN: classList]
}
...
async function perform(endpoint, data, isFile = false) {
  ...
  if (per && per.alignedRef && per.alignedQry && per.ops && perBox && perDiv) {
    ...
    console.log("Per-position track not rendered (missing data or elements)."); // [MDN: getElementById]
  }
...
// Rendering helpers
function renderPerPos(alnRef, alnQry, ops, wrap = 40) {
  const rows = [];
